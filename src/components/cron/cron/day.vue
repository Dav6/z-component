<template>
    <div class="cron-item day" :val="_value">
        <div>
            <d-el-radio v-model="type"
                        :data="{isRadioBorder:true,options:[{value:'1',label:`每${unit}`}]}"></d-el-radio>
        ...
        <div>
            ...
            <span style="margin-left: 10px; margin-right: 5px;">从</span>
            <d-el-input-number @change="goTo('setType','3')" v-model="loop.start"
                               :data="{min:1,max:31,}"></d-el-input-number>
            <span style="margin-left: 5px; margin-right: 5px;">{{ unit }}开始，每</span>
            <d-el-input-number @change="goTo('setType','3')" v-model="loop.end"
                               :data="{min:1,max:31,}"></d-el-input-number>
        ...
        <div>
            <d-el-radio v-model="type"
                        :data="{isRadioBorder:true,options:[{value:'6',label:`本月最后一天`}]}"></d-el-radio>
...
const onTypeChange = async (preType) => {

    const _preType = preType;
    const _type = type.value

    let _week = props.cronData?.find(item => item.key == 'week') || {};
    // console.log('props.cronData', _week)
    // console.log('d_type---', _type)
    // console.log('_preType',_preType)
    const _unspecified = `<span style="font-weight: bold">不指定</span>`

    let _message = `日与周必须有一个为${_unspecified} <br>将周设置为${_unspecified}`;
    if (_week.value != '?' && _type != '5') {
         showMessage(_message).then(res=>{
    ...
    if (_week.value == '?' && _type == '5') {
        _message = `日与周不可以同时为${_unspecified}</br>将周设置为每周`;
         showMessage(_message).then(res=>{
        },err=>{
            // console.log('失败2')
            setTimeout(()=>{
...
watch(() => type.value, (type, preType) => {

    console.log('day-watch-type', type)
    onTypeChange(preType)
...
const updateVal = () => {
    // console.log('updateVal-day', props.modelValue)
