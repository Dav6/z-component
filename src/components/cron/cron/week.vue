<template>
    <div class="cron-item month" :val="_value">
        ...
        <div>
            <d-el-radio v-model="type"
                        :data="{isRadioBorder:true,options:[{value:'6',label:'本月最后一个'}]}"></d-el-radio>
...
//       // console.log('week-day',_day);
//       // console.log('week-_type.value',value )
//
//         const _unspecified = `<span style="font-weight: bold">不指定</span>`
//
//       let _message = `周与日必须有一个为${_unspecified}<br>将日设置为${_unspecified}`;
//       if (_day.value != '?' && value != '5') {
//         await showMessage(_message);
//
//       }
//       if (_day.value == '?' && value == '5') {
//         _message = `周与日不可以同时为${_unspecified}</br>将日设置为每日`;
//         await showMessage(_message);
//
//       }
//
